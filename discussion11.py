...
def create_patients_table(cur, conn):
    ...
    cur.execute("CREATE TABLE Patients (id INTEGER PRIMARY KEY, name TEXT, \species_id INTEGER, age INTEGER, cuteness INTEGER, agressiveness INTEGER)")
    conn.commit()

5
# ADD FLUFFLE TO THE TABLE
def add_fluffle(cur, conn):
    cur.execute("INSERT INTO Patients (id, name, species_id, age, cuteness, agressiveness) \ VALUES (?,?,?,?,?,?)") ('0', 'Fluffle', '0', '3', '90', '100')
    conn.commit()
...
def add_pets_from_json(filename, cur, conn):
    ...
    for x in json_data:
        ...
        cur.execute("INSERT INTO Patients (id, name, species_id, age, cuteness, aggressiveness) \ VALUES (?,?,?,?,?,?", (id, pet_name, species, age, cuteness, aggressiveness))
        pet_count += 1
    conn.commit()
